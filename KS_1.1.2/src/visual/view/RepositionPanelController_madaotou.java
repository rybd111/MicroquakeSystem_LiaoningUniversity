	...
	void onSure(ActionEvent event) throws MWException, ParseException {
		...
		for (int i = 0; i < numbername.size(); i++) {
			...
			if (numbername.get(i).equals("O sel ")) {
				senserInformation[i][0] = pHashMap.get('O') / (Parameters.FREQUENCY + 200)-t;// 相对时间???
			...
			if (numbername.get(i).equals("P nhy")) {
				senserInformation[i][0] = pHashMap.get('P') / (Parameters.FREQUENCY + 200)-t;
			...
			if (numbername.get(i).equals("Q wmz ")) {
				senserInformation[i][0] = pHashMap.get('Q') / (Parameters.FREQUENCY + 200)-t;
